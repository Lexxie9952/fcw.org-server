patch -p1 -u -R -s --dry-run -d .. <Remove-Google-Authorization-in-Longturn.patch >/dev/null 2>/dev/null
if [ $? -ne 0 ];then
  ...
  patch -p1 -u -R -s -d .. <Remove-Google-Authorization-in-Longturn.patch
  if [ $? -eq 0 ];then
    ...
    echo "Failed to restore Google authorization!"
    exit 1
  fi
fi
