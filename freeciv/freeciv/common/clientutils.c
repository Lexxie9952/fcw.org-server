...
/************************************************************************//**
  ...
  FIXME: This FCW addition of turns pointer is buggy,
    ...
    if (is_build_activity(i, ptile)) {
      extra_type_iterate(ep) {
        ...
        if (calc->extra_turns[ei][i] > 0) {
          ...
          *turns = calc->extra_turns[ei][i]; /* FCW bug, see function header */
          num_activities++;
        }
      } extra_type_iterate_end;
      ...
      if (rmcause != ERM_NONE) {
        extra_type_by_rmcause_iterate(rmcause, ep) {
          ...
          if (calc->rmextra_turns[ei][i] > 0) {
            ...
            *turns = calc->rmextra_turns[ei][i]; /* FCW bug, see function header */
            num_activities++;
          }
      ...
      if (calc->activity_turns[i] > 0) {
        ...
        *turns = calc->activity_turns[i]; /* FCW bug, see function header */
        num_activities++;
      }
    }
  } activity_type_iterate_end;
