					...
					int counterpart)
{
  ...
  *player_accept = ! *player_accept;
  ...
  if (ptreaty->accept0 && ptreaty->accept1) {
    ...
    notify_player(pplayer, NULL, E_DIPLOMACY, ftc_server,
                  PL_("[`scroll`] A treaty containing %d clause was agreed upon.",
                      "[`scroll`] A treaty containing %d clauses was agreed upon.",
                      nclauses),
                  nclauses);
	...
	case CLAUSE_CITY:
	  ...
	  if (!pcity) { /* Can't find out cityname any more. */
            notify_player(pplayer, NULL, E_DIPLOMACY, ftc_server,
                          _("[`scroll`] One of the cities the %s are giving away"
	  ...
	  if (city_owner(pcity) != pother) {
            notify_player(pplayer, NULL, E_DIPLOMACY, ftc_server,
                          _("[`scroll`] The %s no longer control %s! "
                            "Treaty canceled!"),
                          nation_plural_for_player(pother),
                          city_link(pcity));
            notify_player(pother, NULL, E_DIPLOMACY, ftc_server,
  ...
  case CLAUSE_CEASEFIRE:
	...
	case CLAUSE_GOLD:
	  if (pother->economic.gold < pclause->value) {
            notify_player(pplayer, NULL, E_DIPLOMACY, ftc_server,
                          _("[`scroll`] The %s don't have the promised amount "
    ...
    clause_list_iterate(ptreaty->clauses, pclause) {
      ...
      case CLAUSE_EMBASSY:
        ...
        notify_player(pgiver, NULL, E_TREATY_EMBASSY, ftc_server,
                      _("[`scroll`] You gave an embassy to %s."),
                      player_name(pdest));
        notify_player(pdest, NULL, E_TREATY_EMBASSY, ftc_server,
                      _("[`scroll`] %s allowed you to create an embassy!"),
                      player_name(pgiver));
        break;
      case CLAUSE_ADVANCE:
        {
          ...
          if (tech_transfer(pdest, pgiver, pclause->value)) {
            ...
            notify_research(presearch, pdest, E_TECH_GAIN, ftc_server,
                            _("[`bulb`] You have acquired %s%s thanks to the %s "
                              "treaty with the %s."),
            ...
            notify_research_embassies
                (presearch, pgiver, E_TECH_EMBASSY, ftc_server,
                 ...
                 Q_("?fromplr:[`bulb`] The %s have acquired %s%s from the %s."),
                 research_name,
                 (game.server.blueprints ? _("blueprints for ") : _("")),
                 advance_name,
                 nation_plural_for_player(pgiver));
	...
	give_map_from_player_to_player(pgiver, pdest);
        notify_player(pdest, NULL, E_DIPLOMACY, ftc_server,
                      ...
                      _("[`scroll`] You receive the %s worldmap."),
	...
	give_seamap_from_player_to_player(pgiver, pdest);
        notify_player(pdest, NULL, E_DIPLOMACY, ftc_server,
                      /* TRANS: ... Polish seamap. */
                      _("[`scroll`] You receive the %s seamap."),
                      nation_adjective_for_player(pgiver));
      ...
      case CLAUSE_CEASEFIRE:
        ...
        if (!reaffirmation) {
          notify_player(pgiver, NULL, E_TREATY_CEASEFIRE, ftc_server,
                        _("[`scroll`][`olivebranch`] You agree on a cease-fire with %s."),
                        player_name(pdest));
          notify_player(pdest, NULL, E_TREATY_CEASEFIRE, ftc_server,
            ...
            notify_player(pgiver, NULL, E_TREATY_CEASEFIRE, ftc_server,
                        _("[`scroll`][`olivebranch`] You affirm a cease-fire with %s, agreeing to reset casus belli."),
                        player_name(pdest));
            notify_player(pdest, NULL, E_TREATY_CEASEFIRE, ftc_server,
                        _("[`scroll`][`olivebranch`] You affirm a cease-fire with %s, agreeing to reset casus belli."),
                        player_name(pgiver));
	...
	break;
      case CLAUSE_PEACE:
        ...
        notify_player(pgiver, NULL, E_TREATY_PEACE, ftc_server,
                      ...
                      PL_("[`scroll`][`dove`] You agree on an armistice with the %s. In %d turn, "
                          "it will become a peace treaty. Move your "
                          "military units out of %s territory to avoid them "
                          "being disbanded.",
                          "[`scroll`][`dove`] You agree on an armistice with the %s. In %d turns, "
	...
	break;
      case CLAUSE_ALLIANCE:
        ...
        notify_player(pgiver, NULL, E_TREATY_ALLIANCE, ftc_server,
                      _("[`scroll`][`alliance`] You agree on an alliance with %s."),
                      player_name(pdest));
	...
	give_shared_vision(pgiver, pdest);
        notify_player(pgiver, NULL, E_TREATY_SHARED_VISION, ftc_server,
                      _("[`scroll`][`eye`] You give shared vision to %s."),
                      player_name(pdest));
        notify_player(pdest, NULL, E_TREATY_SHARED_VISION, ftc_server,
                      _("[`scroll`][`eye`] %s gives you shared vision."),
                      player_name(pgiver));
				       ...
				       int counterpart)
{
  ...
  if (get_player_bonus(pplayer, EFT_NO_DIPLOMACY) > 0
    ...
    notify_player(pplayer, NULL, E_DIPLOMACY, ftc_server,
                  _("[`skull`] Your diplomatic envoy was decapitated!"));
    return;
  }
